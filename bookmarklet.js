...
(function() {
  ...
  // Add slide-in animation (check if style already exists to prevent duplicates)
  if (!document.getElementById('textify-animation-style')) {
    const style = document.createElement('style');
    style.id = 'textify-animation-style';
    style.textContent = `
      @keyframes slideIn {
        from {
  ...
  messageHandler = function(event) {
    ...
    if (event.data.action === 'extractPage') {
      // Extract page data (async)
      extractPageData().then(pageData => {
        // Guard against null iframe.contentWindow
  ...
  // Check for obvious metadata/navigation text
