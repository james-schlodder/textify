...
(function() {
  ...
  function extractTextFromElement(element) {
    ...
    const unwantedSelectors = [
      ...
      // NYTimes specific elements
      '.css-at9mc1', '.css-1fanzo5', '.css-53u6y8', '.css-1hwfws3',
      '[data-testid="photoviewer-wrapper"]', '[data-testid="photo-credit"]',
      '.StoryHeader-byline', '.byline', '[aria-label="tools"]',
      '.css-1cp3ece', '.css-1r6w7i', '.css-175oi2r'
    ];
    ...
    // Clean up common pre-article patterns
    text = cleanPreArticleContent(text);
    
    return text.trim();
  }
  
  // Function to clean up content that appears before and after the actual article
  function cleanPreArticleContent(text) {
    // Clean up common pre-article patterns
    text = removePreArticlePatterns(text);
    
    // Clean up common post-article patterns
    text = removePostArticlePatterns(text);
    
    // Remove multiple consecutive whitespace characters
    text = text.replace(/\s+/g, ' ');
    
    // Advanced cleanup: Look for the actual article start and end
    text = findArticleStart(text);
    text = findArticleEnd(text);
    
    return text.trim();
  }
  
  // Remove content that appears before the article
  function removePreArticlePatterns(text) {
    const preArticlePatterns = [
      // Universal credit patterns (works for most news sites)
      /Credit\.{3}[^\.]+\./g,
      /Photo Illustration by[^\.]+\./g,
      /source photographs by[^\.]+\./g,
      /Photo credit[^\.]+\./g,
      /Image credit[^\.]+\./g,
      
      // Universal navigation patterns
      /Supported by[A-Z\s]+/g,
      /SKIP ADVERTISEMENT/g,
      /Listen to this article[^\.]*\./g,
      /Learn more[A-Z][a-z]*/g,
      /Share full article\d*/g,
      /Share this article/g,
      
      // Universal byline patterns
      /By [A-Z][a-z]+ [A-Z][a-z]+( and [A-Z][a-z]+ [A-Z][a-z]+)?/g,
      /[A-Z][a-z]+ [A-Z][a-z]+ reported from[^\.]+\./g,
      
      // Universal date patterns
      /Published [A-Z][a-z]+ \d+, \d+/g,
      /Updated [A-Z][a-z]+ \d+, \d+/g,
      /Published: [^\.]+/g,
      /Last updated: [^\.]+/g,
      
      // Universal language and subscription patterns
      /Leer en español/g,
      /Read in [A-Z][a-z]+/g,
      /Subscribe to [^\.]+/g,
      /Sign up for [^\.]+/g,
      
      // Universal ad and UI patterns
      /Advertisement/gi,
      /Subscribe now/gi,
      /Sign in/gi,
      /Create account/gi
    ...
    // Apply each pattern to remove pre-article content
    for (const pattern of preArticlePatterns) {
  ...
  // Remove content that appears after the article
  function removePostArticlePatterns(text) {
    // Post-article patterns - these typically appear at the end
    const postArticlePatterns = [
      // Author contributions and corrections
      /[A-Z][a-z]+ [A-Z][a-z]+ contributed reporting from[^\.]+\./g,
      /A correction was made on[^:]+:[^\.]+\./g,
      /An earlier version of this article[^\.]+\./g,
      /When we learn of a mistake[^\.]+\./g,
      /If you spot an error[^\.]+\./g,
      
      // Author bios and metadata
      /Learn more[A-Z][a-z]+ [A-Z][a-z]+ is [^\.]+\./g,
      /[A-Z][a-z]+ [A-Z][a-z]+ is a [^\.]+correspondent[^\.]+\./g,
      /[A-Z][a-z]+ [A-Z][a-z]+ is the [^\.]+correspondent[^\.]+\./g,
      /Previously (he|she) wrote about[^\.]+\./g,
      
      // Print and subscription info
      /A version of this article appears in print[^\.]+\./g,
      /Order Reprints \| Today's Paper \| Subscribe/g,
      /See more on:[^\.]+\./g,
      /Read \d+ comments/g,
      /Share full article\d*/g,
    ...
    // Apply each pattern to remove post-article content
    for (const pattern of postArticlePatterns) {
      text = text.replace(pattern, '');
    }
  ...
  // Function to intelligently find where the actual article content ends
  function findArticleEnd(text) {
    // Common phrases that signal the end of article content
    const endSignals = [
      // Author attribution that's not part of the story
      /[A-Z][a-z]+ [A-Z][a-z]+ contributed reporting/,
      /A correction was made/,
      /Learn more[A-Z][a-z]+ [A-Z][a-z]+ is/,
      /[A-Z][a-z]+ [A-Z][a-z]+ is a .* correspondent/,
      /A version of this article appears/,
      /See more on:/,
      /Read \d+ comments/
    ];
    
    // Find the earliest occurrence of any end signal
    let earliestEnd = text.length;
    
    for (const signal of endSignals) {
      const match = text.search(signal);
      if (match !== -1 && match < earliestEnd) {
        earliestEnd = match;
      }
    }
    
    // If we found an end signal, cut the text there
    if (earliestEnd < text.length) {
      text = text.substring(0, earliestEnd);
    }
    
    // Additional cleanup: remove trailing partial sentences that might be metadata
    const sentences = text.split(/[.!?]+/);
    if (sentences.length > 1) {
      const lastSentence = sentences[sentences.length - 1].trim();
      
      // If the last sentence looks like metadata, remove it
      if (lastSentence.length < 20 || 
          /^(See|Read|Share|Learn|Order|Subscribe|Advertisement)/i.test(lastSentence) ||
          /contributed|correction|version|reprints/i.test(lastSentence)) {
        sentences.pop();
        text = sentences.join('.') + '.';
      }
    }
  ...
  // Function to intelligently find where the actual article content starts
  function findArticleStart(text) {
    const words = text.split(' ');
    if (words.length < 10) return text;
    
    // Look for common article starting patterns (universal across news sites)
    const articleStartPatterns = [
      // Location-based starts (very common in news)
      /\b(In|At|On|From|During|After|Before|Since|While|Near|Across|Throughout)\s+[A-Z]/,
      // Direct statements
      /\b[A-Z][a-z]+\s+(is|was|has|had|will|would|could|should|may|might|said|announced|reported)/,
      // Quote starts
      /\b"[A-Z]/,
      // Company/organization names
      /\b[A-Z][a-z]+(\s+[A-Z][a-z]+)*\s+(Inc\.|Corp\.|Co\.|LLC|Ltd\.|announced|said|reported)/,
      // Numbers and data
      /\b(More than|Less than|About|Nearly|Almost|Over|Under|Approximately)\s+\d/,
      /\b\d+\s+(percent|million|billion|thousand|years?|months?|days?)/,
      // Government and officials
      /\b(President|Prime Minister|CEO|Officials|Authorities|Government|Congress|Senate)\s/,
      // Time-based starts
      /\b(Yesterday|Today|This week|Last month|On [A-Z][a-z]+)\s/
    ];
    
    // Look through the text to find a good starting point
    for (let i = 0; i < Math.min(words.length, 80); i++) {
      const remainingText = words.slice(i).join(' ');
      
      // Check if this position looks like an article start
      for (const pattern of articleStartPatterns) {
        if (pattern.test(remainingText)) {
          // Verify this isn't just a fragment by checking length
    ...
    // Fallback: remove common pre-article indicators more aggressively
    const fallbackCleanup = [
      /.*?(Leer en español|Read in [A-Z][a-z]+)\s*/i,
      /.*?(Updated|Published)\s+[A-Za-z]+ \d+,? \d+\s*/,
      /.*?reported from[^\.]+\.\s*/,
      /.*?(By|Author:)\s+[A-Z][a-z]+ [A-Z][a-z]+[^\.]*\.\s*/,
      /.*?(SKIP ADVERTISEMENT|Advertisement)\s*/i,
      /.*?(Subscribe|Sign up|Learn more)\s*/i
    ];
    
    let cleanText = text;
    for (const pattern of fallbackCleanup) {
      const cleaned = text.replace(pattern, '');
      if (cleaned !== text && cleaned.length > 100 && cleaned.trim()) {
        cleanText = cleaned;
        break;
      }
    }
